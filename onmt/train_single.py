...
def main(opt, device_id):
    ...
    for feat in sorted(f for f in fields if 'src_feat' in f):
        logger.info(' * {} size = {}'.format(feat, len(fields[feat].vocab)))
    for feat in sorted(f for f in fields if 'tgt_feat' in f):
        logger.info(' * {} size = {}'.format(feat, len(fields[feat].vocab)))
